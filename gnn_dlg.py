...
def dlg_attack_gnn(model, target_gradients, data, target_idx, true_label, num_iterations=200, attack_lr=0.05, num_restarts=1):
    ...
    for restart in range(num_restarts):
        ...
        optimizer = torch.optim.LBFGS([dummy_x], lr=attack_lr, max_iter=20, history_size=20, line_search_fn='strong_wolfe')
        ...
        for iters in range(max(1, num_iterations // 20)):
            ...
            if iters % 5 == 0 or iters == (num_iterations // 20) - 1:
                print(f"L-BFGS Step {iters*20:3d}/{num_iterations} | Loss: {current_loss:.6f}")
    ...
    return best_dummy_x, mse, cosine_sim
